...
> 이 글은 AI 시대 한국 보안 거버넌스 시리즈의 5편이자 국가 전략 확장편입니다. 앞선 글들이 평가 체계, 다행위자 균형, 적응 능력 지표, 시장 압력을 다뤘다면, 이 글은 그 논의를 AI control plane 주권과 국가 AI 보안 운영 체계로 확장합니다.
>
> - 1편: [한국 보안 거버넌스는 AI 시대에 잘못된 방향으로 가속되고 있다](/ko/post/2026-04-26-ai-security-governance-korea/)
> - 2편: [한국 보안 거버넌스는 왜 바뀌지 않는가](/ko/post/2026-04-30-why-korean-security-governance-does-not-change/)
...
> - 5편: **AI 국가 전략의 진짜 승부처는 GPU 수량만이 아닙니다**
...
이 점은 AI exploit benchmark에서도 반복해서 드러납니다. 모델의 성능은 agent loop, tool interface, prompt, nudge, context budget, grading 방식과 분리해서 봐야 하며, 비용 효율도 단순 token price가 아니라 하니스가 얼마나 결과를 끌어올리는지까지 함께 봐야 합니다.
...
Anthropic Glasswing도 비슷한 방향으로 해석할 수 있습니다. 차이는 대상이 운영체제와 애플리케이션 생태계가 아니라 AI 보안 검증 생태계라는 점입니다.

물론 AI control plane은 Windows처럼 단일 사업자가 완전히 통제하는 폐쇄형 플랫폼과는 다릅니다. 모델, 클라우드, agent framework, 데이터, 인증, disclosure, 산업별 규제가 훨씬 더 분산되어 있기 때문입니다. 그래서 앞으로의 권력은 단일 독점보다 표준, 인증, 하니스, 파트너 네트워크, disclosure 조정 능력을 가진 거버넌스 노드에 가까울 가능성이 큽니다.

AI 시대의 플랫폼 권력은 모델을 가진 쪽에만 있지 않습니다. 하니스를 가진 쪽, 검증 기준을 가진 쪽, 파트너 네트워크를 가진 쪽, 취약점 공개와 패치 흐름을 조율하는 쪽으로 이동하고 있습니다.
...
한국이 이 축을 놓치기 쉬운 데에는 구조적 이유가 있습니다. 한국 산업정책은 반도체, 제조, 데이터센터, 전력 인프라처럼 눈에 보이는 물리 기반을 설명하고 투자하는 데 익숙합니다. 반면 AI assurance, 보안 하니스, agent 권한 경계, tool-call 로그, disclosure coordination은 성과가 즉시 보이지 않고, 아직 독립된 산업 언어로도 충분히 정리되어 있지 않습니다. 그래서 컴퓨트 인프라 투자는 국가 전략의 중심에 오르기 쉽지만, 그 위에서 작동하는 검증과 운영 증적 체계는 뒤로 밀리기 쉽습니다.
...
이를 거대한 구호로만 둘 필요는 없습니다. 첫 단계는 공공·금융·제조 AI 조달 기준에 몇 가지 운영 증적을 포함하는 것에서 시작할 수 있습니다. 예를 들어 model 구성 정보, dataset lineage, RAG source provenance, agent 권한 범위, tool-call 로그 보존, 사고 시 포렌식 접근권은 조달과 인증 과정에서 검토할 수 있는 항목입니다. 산업별로는 제조 AX, 금융 AI, 공공 AI에 맞는 재현 가능한 red team 하니스와 assurance 기준이 필요합니다. AI-generated vulnerability finding을 접수하고, 중복 제거하고, 우선순위화하고, 패치 검증까지 연결하는 국내 검증 역량도 함께 키워야 합니다.
...
앞서 말한 위험 중에서도 가장 빨리 굳어질 수 있는 것은 agent 권한과 tool-call 로그입니다. 모델과 GPU는 조달 주기가 길지만, AI IDE, MCP, SaaS connector, workflow agent는 훨씬 빠르게 현장에 들어옵니다. 이때 어떤 agent가 어떤 시스템을 호출할 수 있는지, 어떤 승인을 거쳤는지, 어떤 결과와 증적을 남겼는지가 외부 플랫폼의 기본값으로 굳어지면 나중에 되돌리기 어렵습니다. AI control plane 주권의 첫 전선은 agent가 도구를 호출하는 순간의 권한과 로그입니다.
...
이 글은 AI control plane을 국가 전략의 문제로 다뤘지만, 같은 논의는 취약점 대응과 개발 도구 공급망에서도 반복된다.

- [CVE 이후 대응만으로는 늦다: AI 시대 취약점은 번호가 붙기 전에 움직인다](/ko/post/2026-04-29-after-cve-response-ai-vulnerability/) — AI 시대에는 취약점이 번호를 받기 전부터 움직이며, 대응 속도와 검증 흐름이 중요해진다.
- [AI Slop의 역설: 취약점을 더 잘 찾는 시대에 triage가 더 어려워지는 이유](/ko/post/2026-04-30-ai-slop-vulnerability-triage/) — AI가 후보를 많이 만들수록 중복 제거, 우선순위화, 검증 하니스가 control plane의 핵심이 된다.
- [보안진단은 외주 업무가 아니라 개발 공정이 된다](/ko/post/2026-05-01-security-assessment-as-development-process/) — 보안 검증은 일회성 리포트가 아니라 개발 공정 안에서 반복되는 운영 체계가 되어야 한다.
- [공급망 보안은 SBOM만으로 끝나지 않는다: AI 개발 도구와 자동화 연결 관리](/ko/post/2026-05-02-ai-development-tools-supply-chain-governance/) — AI IDE, MCP, 자동화 커넥터는 개발 프로세스의 신뢰 경로이자 공급망 자산이다.
- [MCP는 RPC 보안의 역사를 반복하고 있다](/ko/post/2026-05-07-mcp-is-repeating-rpc-security-history/) — MCP는 AI control plane 경계의 구체적 사례다. 외부 context와 configuration이 내부 tool execution authority로 바뀌는 지점이기 때문이다.
...
- 방법론 근거 노트: “ExploitBench 해석: ExploitGym의 attribution 문제를 capability ladder와 neutral runner로 보완한다”.  
...
- 방법론 근거 노트: “ExploitBench 비용 효율 해석: GLM은 싼 모델이 아니라 하네스로 증폭할 cheap exploration engine이다”.  
