...
- 이 저장소는 `fact / premise / hypothesis / conclusion` 분리, 랩 개선 여부 판단, 주장 강도 조절 같은 상위 실험 해석 워크플로를 제공합니다.
...
| **WCD Testing Tool** | [Ap6pack/web-cache-deception-testing-tool](https://github.com/Ap6pack/web-cache-deception-testing-tool) | 중간 계층-후단 정규화 | 6개 정적 확장자(.css, .webp, .webp, .txt, .html, .ico)와 8개 delimiter(`;@,!~%#?`)를 조합하여 캐시 적중 여부 자동 확인. 인증/비인증 요청 비교로 Cross-user 노출 탐지 |
...
```bash
# 6개 확장자(.css, .webp, .webp, .txt, .html, .ico) × 8개 delimiter(; @ , ! ~ % # ?) 자동 테스트
...
**목적**: chunk extension과 trailer header뿐 아니라 quoted-string CRLF, escaped LF/CR 변형이 request-boundary anomaly로 이어지는지 확인
...
**확장 점검 포인트**:
- `quoted_string_crlf`, escaped LF/CR 같은 변형은 단순 chunk extension/trailer 누락과 별도 결과로 기록한다.
- 결과는 최소한 `(1) single-request anomaly`, `(2) hidden second request execution`, `(3) response-queue poisoning`, `(4) fan-out / availability pressure`로 분리해서 적는다.
- hidden second request 실행이 관찰되더라도, 피해자 응답 소유권이 입증되지 않았다면 cross-user desync나 세션 혼선으로 바로 승격하지 않는다.
- same-IP 또는 same-host 하네스는 concurrency/fan-out ceiling을 과소평가하거나 랩 artifact를 만들 수 있으므로, fan-out/DoS 성격 주장은 distinct client IP 또는 격리 namespace로 재실행한 뒤 기록한다.
- 안정적인 ceiling 또는 DoS 수치는 fresh-lab 또는 cooldown 이후 재측정한 값만 사용한다.

**권장 자동화**:
- `run_tc24_smuggling_probe.py`: `quoted_string_crlf`, escaped LF/CR, multi-response marker(`status_chain`) 확인
- `run_tc24_multiip_probe.sh`: 로컬 Docker lab에서 multi-client fan-out 재검증

**판정 기준**:
- WAF는 chunk extension/trailer를 무시하고 backend만 처리하면 → parser discrepancy
- trailer가 security-relevant metadata처럼 동작하면 → 후단 해석 불일치
- 한 요청에 대해 복수 응답 마커가 보이거나 숨은 후속 요청이 실행되면 → request-boundary anomaly로 기록
- response-queue poisoning은 피해자 연결에서 공격자 소유 응답이 실제로 관찰된 경우에만 결론낸다.
- fan-out pressure는 multi-client 재실행과 fresh-lab/cooldown 재검증까지 통과했을 때만 안정적인 영향도로 기록한다.
...
최신 재검증 워크플로는 `$waf-ips-ids-retest`를 실행 계층으로, `$security-hypothesis-lab`을 해석 계층으로 사용한다. 핵심은 "많이 때려보는 것"보다 **무엇이 확인된 사실이고, 무엇이 아직 가설인지 분리하는 것**이다.

- `fact`: 캡처, 로그, 응답 fingerprint, pcap, 디버그 로그처럼 직접 관찰된 사실
- `premise`: 프록시 체인, 백엔드 종류, cache placement, same-IP bias처럼 아직 확인되지 않은 전제
- `hypothesis`: 다음 실험으로 검증할 단일 인과 주장
- `conclusion`: 전제를 확인하거나 랩을 개선한 뒤에도 남는 결론

실무적으로는 다음 원칙이 중요하다.

- stronger impact가 추가 전제에 의존하면, **입증된 primitive와 조건부 impact를 분리해서** 적는다.
- 현재 랩으로 진짜 질문에 답할 수 없으면, 결론을 늘리지 말고 **랩을 개선하거나 주장을 낮춘다**.
- same-IP, same-host, warm lab 상태가 결과를 흔들 수 있으면 재실행보다 먼저 bias를 줄이는 랩 보정을 수행한다.
- 결론 강도는 최소한 `target-proven`, `lab-proven`, `hypothesis-only`처럼 단계화해서 쓰는 것이 안전하다.
...
향후 탐지 체계는 다음 **열세 가지 방향**으로 고도화되어야 하며, 실무 우선순위에 따라 정렬한다.
...
12. **가설 기반 검증 루프 정착**: fact, premise, hypothesis, conclusion을 분리하지 않으면 parser discrepancy와 smuggling 결과를 과대해석하기 쉽다. noisy run은 단순 반복보다 랩 개선, ownership 증거 확보, 또는 주장 강도 하향 중 무엇이 필요한지 먼저 판단해야 한다.

13. **결론 강도와 smuggling impact 분리**: request-boundary anomaly, hidden second request execution, response-queue poisoning, cross-user confusion, fan-out pressure는 같은 말이 아니다. same-IP 편향과 warm-lab artifact를 제거한 뒤에도 남는 결과만 `target-proven`으로 승격하고, 그 외는 `lab-proven` 또는 `hypothesis-only`로 남겨야 한다.
