...
## 9. 부록 — LLM과 함께 이 분석을 할 때 진짜 병목

이 글의 익스플로잇은 한 번에 나온 게 아니다. 가설을 세우고, 실측으로 뒤집고, 다시 좁히는 과정을 여러 번 반복했다. 그 과정에서 배운 건, LLM과 함께 취약점을 분석할 때 **가장 큰 병목이 고급 기술 지식의 부재가 아니라는 것**이었다. 진짜 병목은 **그럴듯한 설명을 너무 빨리 믿어버리는 것**이다.

대상은 웹 서버가 서드파티 서버와 통신하려고 띄우는 서버측 네이티브 헬퍼 바이너리였다. 진짜 관문은 "오버플로우가 된다"를 보이는 게 아니라, **ASLR이 켜진 상태에서 같은 프로세스 안에서 체인이 이어짐**을 보이는 것이었다 — leak → Stage2 → pivot → 실행 확인.
...
1. **GDB 관측 한 번은 증거가 아니다.** 브레이크포인트에서 한 번 본 레지스터 값이나 크래시 상태는, 반복 실행·strace·메모리 덤프·런타임 맵으로 확인하기 전까지는 가설일 뿐이다.
2. **recv를 fd·buf·len만으로 분석하지 마라.** 네 번째 인자 flags도 동작을 바꾸고 EAGAIN을 유발할 수 있다. fd·buf·len·flags·반환값·errno를 함께 봐라.
3. **PLT나 libc가 인자를 대신 정리해줄 거라 가정하지 마라.** 정상 래퍼는 인자를 맞게 준비하지만, 직접 ROP나 함수 중간 진입은 그 혜택을 못 받는다. 함수 인자는 호출자의 책임이다.
4. **정적 ELF 정보와 런타임 매핑을 분리하라.** readelf·objdump의 섹션·세그먼트로는 부족하다. 실행 중 프로세스에서 실제로 매핑·쓰기 가능한 영역을 확인하라.
5. **내부 래퍼부터 찾아라.** libc나 syscall 호출을 새로 짜기 전에, 바이너리에 select·timeout·recv flags·인자 세팅을 위한 안정적인 경로가 이미 있는지 확인하라.
6. **ASLR이 켜졌으면 같은 프로세스 연속성을 검증하라.** 유출한 주소는 그 프로세스 인스턴스 안에서만 유효할 수 있다. leak → Stage2 → pivot → 실행 확인이 같은 프로세스에서 일어나야 한다.
7. **확정 사실과 가정을 분리하라.** 결론을 네 칸으로 적어라 — 확정된 사실, 반증된 가설, 미지, 다음 실험.

이 규칙들은 추상론이 아니다. 규칙 2·5(recv flags와 내부 래퍼)는 이 저장소의 **03(ret2csu + flags=0)**이 그대로 실증하고, 규칙에 나온 "ROP 체인 배치 오해"는 **02(front-load)**가, "같은 프로세스 연속성"은 **01**이 코드로 보여준다. 즉 이 글의 실패들이 곧 세 개의 러너블 랩으로 굳어졌다.
